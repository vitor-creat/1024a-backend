...
routes.post("/cadastro_produtso", async (req, res) => {
  const { id, nome, categoria, preco, data_criacao, data_modificacao } =
    req.body;
  try {
    const [result] = await connection.execute<ResultSetHeader>(
...
routes.get("/quantidade_pedidos", async (req, res) => {
  try {
    const [dados, campos] = await connection.execute<IPedidos[]>(
      "SELECT COUNT(*) AS quantidade_pedidos FROM itenspedidos",
    );
    res.status(200).json(...dados);
  } catch (err) {
    const mysqlErrorHandle = new MysqlErrorHandle(err, res);
...
routes.post("/cadastro_produto_v2", async (req, res) => {
  ...
  try {
    const [result] = await connection.execute<ResultSetHeader>(
      ...
      [id, nome, categoria, preco, data_criacao, null],
    );
    res.status(201).json({ mensage: "sucesso ao inserir" });
  } catch (err) {
    const mysqlErrorHandle = new MysqlErrorHandle(err, res);
...
routes.put("/produto_preco/:id", async (req, res) => {
  ...
  try {
    const [result] = await connection.execute<ResultSetHeader>(
      "UPDATE produto SET nome = ?, preco = ?, categoria = ?, data_modificacao = ? where id = ?",
...
routes.put("/produto_completo/:id", async (req, res) => {
  ...
  let data_modificacao = new Date();
  try {
    const [dados, campos] = await connection.execute<ResultSetHeader>(
      "SELECT * FROM produto where id = ?",
      [id],
    );

    if (dados.affectedRows == 0) {
      return res.status(400).json({ mensagem: "Deu certo" });
    }
